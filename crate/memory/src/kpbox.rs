...
use os_units::Bytes;
use x86_64::{
    structures::paging::{RecursivePageTable, Size4KiB, Translate},
    PhysAddr, VirtAddr,
};
...
impl<T: ?Sized> KpBox<T> {
    ...
    pub fn phys_addr(&self) -> PhysAddr {
        let page_table = RecursivePageTable::new(active_level_4_table()).unwrap();
        page_table
    ...
    fn from_bytes(bytes: Bytes) -> Self {
        let virt = allocate_new(bytes.as_num_of_pages()).expect("Failed to allocate pages.");
