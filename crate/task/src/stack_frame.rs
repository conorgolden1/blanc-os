...
impl StackFrame {
    ...
    fn new(entry: VirtAddr, stack_pointer: VirtAddr, segs: &Selectors) -> Self {
        ...
        let instruction_pointer = VirtAddr::new((loader as usize).try_into().unwrap());
...
impl Selectors {
    fn kernel() -> Self {
        Self::new(GDT.1.kernel_code_selector, GDT.1.kernel_data_selector)
    }

    fn user() -> Self {
        Self::new(GDT.1.user_code_selector, GDT.1.user_data_selector)
    }
